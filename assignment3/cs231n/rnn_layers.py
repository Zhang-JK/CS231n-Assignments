...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    dtanh = (1 - cache[5] ** 2) * dnext_h
    db = np.sum(dtanh, axis=0)
    dx = np.dot(dtanh, cache[2].T)
    dprev_h = np.dot(dtanh, cache[3].T)
    dWx = np.dot(cache[0].T, dtanh)
    dWh = np.dot(cache[1].T, dtanh)
    pass
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    ai = a[:, :a.shape[1]//4]
    af = a[:, a.shape[1]//4:a.shape[1]//2]
    ao = a[:, a.shape[1]//2:3*a.shape[1]//4]
    ag = a[:, 3*a.shape[1]//4:]
    i = sigmoid(ai)
    f = sigmoid(af)
    o = sigmoid(ao)
