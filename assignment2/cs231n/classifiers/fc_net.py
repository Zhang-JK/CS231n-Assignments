...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        l1r, cache1r = relu_forward(l1)
        scores, cache2 = affine_forward(l1r, self.params['W2'], self.params['b2'])
        pass
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        loss += (self.reg * np.sum(self.params['W1'] ** 2) + self.reg * np.sum(self.params['W2'] ** 2)) / 2
        d, grads['W2'], grads['b2'] = affine_backward(d, cache2)
        grads['W2'] += self.params['W2'] * self.reg
        d = relu_backward(d, cache1r)
        _, grads['W1'], grads['b1'] = affine_backward(d, cache1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(len(dims)-1):
            ...
            if self.use_batchnorm and i != len(dims)-2:
                self.params[f'gamma{i+1}'] = np.zeros(dims[i+1])+1
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            scores, cache[i] = affine_forward(scores, self.params[f'W{i}'], self.params[f'b{i}'])
            if self.use_batchnorm:
                scores, cache_b[i] = batchnorm_forward(scores, self.params[f'gamma{i}'], self.params[f'beta{i}'], bn_param=self.bn_params[i-1])
            scores, cache_r[i] = relu_forward(scores)
            if self.use_dropout:
                scores, cache_d[i] = dropout_forward(scores, self.dropout_param)
        scores, cache[self.num_layers] = \
            affine_forward(scores, self.params[f'W{self.num_layers}'], self.params[f'b{self.num_layers}'])
        pass
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        for i in range(self.num_layers, 1, -1):
            ...
            d, grads[f'W{i}'], grads[f'b{i}'] = affine_backward(d, cache[i])
            grads[f'W{i}'] += self.params[f'W{i}'] * self.reg
            if self.use_dropout:
                d = dropout_backward(d, cache_d[i-1])
            d = relu_backward(d, cache_r[i-1])
            if self.use_batchnorm:
                d, grads[f'gamma{i-1}'], grads[f'beta{i-1}'] = batchnorm_backward_alt(d, cache_b[i-1])
        loss += self.reg * np.sum(self.params['W1'] ** 2) / 2
        _, grads['W1'], grads['b1'] = affine_backward(d, cache[1])
        grads['W1'] += self.params['W1'] * self.reg
        pass
        ############################################################################
        #                             END OF YOUR CODE                             #
